...
Use the SKU List tab to view and manage all products.`,
  ...
  // Process LPO - handles both Excel and PDF
...
Total (excl. VAT): AED ${parsed.total_excl_vat.toLocaleString()}
VAT (5%): AED ${parsed.total_vat.toLocaleString()}
Total (incl. VAT): AED ${parsed.total_incl_vat.toLocaleString()}
...
Status: ⏳ Awaiting invoice match`,
            ...
            resolve({
              success: false,
              message: `PDF parsing failed: ${error}. Try converting to Excel or enter manually in /lpo`,
              poNumber: "",
            });
          }
        };
      ...
      // Excel processing (existing code)
      reader.onload = async (e) => {
        try {
          const data = new Uint8Array(e.target?.result as ArrayBuffer);
          const workbook = XLSX.read(data, { type: 'array' });
...
function parseQuadrantLpoFromText(text: string): {
  ...
  try {
    ...
    const lineItems: any[] = [];
    const itemMatches = text.matchAll(/(\d+)\s+(\d{10,})\s+(.+?)\s+(\d+)\s+([\d.]+)\s+([\d.]+)\s+([\d,]+)\s+(\d+(?:\.\d+)?)\s*%\s*([\d.,]+)\s+([\d.,]+)/gi);
    
    for (const match of itemMatches) {
      const qty = parseInt(match[4]);
      const unitCost = parseFloat(match[5]);
      const amountExcl = parseFloat(match[7].replace(',', ''));
      const vatPct = parseFloat(match[8]);
      const vatAmt = parseFloat(match[9].replace(',', ''));
      const amountIncl = parseFloat(match[10].replace(',', ''));
